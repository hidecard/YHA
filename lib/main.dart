...
class _UserProfileDialogState extends State<UserProfileDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isDarkMode ? const Color(0xFF2B1A0D).withOpacity(0.9) : Colors.white.withOpacity(0.9),
          borderRadius: BorderRadius.circular(15),
          border: Border.all(color: primaryColor, width: 2),
          boxShadow: [
            BoxShadow(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                      ...
                      backgroundColor: isDarkMode ? Colors.black26 : Colors.grey[300],
                    ...
                    Container(
                      ...
                      child: history.isEmpty
                          ...
                          : ListView.builder(
                              padding: const EdgeInsets.all(8),
                              itemCount: _getDataPoints(history).length,
                              itemBuilder: (context, index) {
                                final dataPoint = _getDataPoints(history)[index];
                                final points = dataPoint['points'];
                                final label = dataPoint['label'];
                                final percentage = (points / maxPoints).clamp(0.0, 1.0);
                                return Card(
                                  elevation: 5,
                                    ...
                                    child: Row(
                                      children: [
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            shape: BoxShape.circle,
                                            color: points >= 0 ? primaryColor : Colors.red,
                                          ),
                                          child: Center(
                                            child: Text(
                                            ...
                                            children: [
                                              ...
                                              LinearProgressIndicator(
                                                ...
                                                valueColor: AlwaysStoppedAnimation<Color>(
                                                  points >= 0 ? primaryColor : Colors.red,
                                                ),
                                                minHeight: 8,
                                              ),
                                            ],
  ...
  List<Map<String, dynamic>> _getDataPoints(List<dynamic> history) {
    if (chartView == 'daily') {
      return history.asMap().entries.map((entry) {
        final points = num.tryParse(entry.value['points'].toString()) ?? 0;
        final label = DateTime.parse(entry.value['timestamp']).toString().substring(0, 10);
      ...
      return sortedWeeks.map((week) {
        return {'points': weeklyTotals[week]!, 'label': week};
      }).toList();
    }
  }
}